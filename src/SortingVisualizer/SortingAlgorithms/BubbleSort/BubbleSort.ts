import getBubbleSortAnimations from "./getBubbleSortAnimations.ts";
import {
...
const BubbleSort = (array: number[], animationSpeed: number): void => {
  ...
  const animations: Animation[] = getBubbleSortAnimations(array);

  for (let i = 0; i < animations.length; i++) {
    const { comparingElement1, comparingElement2, doSwap, isFinalElement, finalElement } = animations[i];
